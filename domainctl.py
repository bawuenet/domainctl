...
def add_record(domain, host, dnstype, rr):
    ...
    records = [y for y in [dict(zip(headers, x)) for x in records]
    ...
    params = {
        'owner': host, 'type': type_table[dnstype], 'RRecord': rr,
...
def remove_record(domain, host, dnstype, rr):
    ...
    for r in records:
        ...
        if r.status_code != 200:
            error(
                f"Not authorized to remove {host} from {domain} due to {r}")
            return False
